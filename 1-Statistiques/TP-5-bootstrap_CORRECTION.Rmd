...
```{r warmup4}
taille <-  seq(from=10, to= 10000, by=1)
logtaille <- seq(from=2, to= 6, by=0.1)
```
...
df <-  data.frame(10^logtaille, moyenne)
#head(df)
plot(df, type="l");points(10^logtaille,1/sqrt(10^logtaille),type='l',col='red')
```
...
tailles <-  seq(from=1, to= 100000, by=1)
moyennes1 <-  c()
moyennes2 <-  c()
...
df <-  data.frame(tailles, quotient)
#df
plot(df, type="l",ylim=c(-50,50))
...
```{r}
maxs = sapply(1:length(quotient),function(subsize){max(abs(quotient[1:subsize]))})
plot(log(tailles),log(maxs))
```


# Mise en place du TP5: bootstrap sur les Pokemons 
...
library(dplyr)
library(ggplot2)
pokedata <- read.csv(file = "data/pokemons.csv")
